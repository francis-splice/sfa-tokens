...
## Most recent change
2026-06-17: Added a stakeholder "PROTOTYPE" marker. A sticky top status bar on every page (hub + both replica pages) reading "Internal design-token study · not indexed · not for distribution", plus a `[Prototype]` tab-title prefix. Verified on desktop (load + scroll) on the dark Tenebra and light Mervyn pages. Hosting context unchanged: live on GitHub Pages (https://francis-splice.github.io/sfa-tokens/), repo https://github.com/francis-splice/sfa-tokens (public), Vercel stays fair-use blocked and is no longer the host.
